...
class MapViewer(QWidget):
	...
	def __init__(self, *args, **kwargs) -> None:
		...
		ch = (wc() if callable(wc) else wc)
		self._channel = ch or QtWebChannel.QWebChannel(self.page)
		# Navigation threading primitives
		self._nav_event = threading.Event()
		self._nav_lock = threading.Lock()
		self.nav_thread = None
		self.target = []
		# Path planner: separate pure planning logic into its own class
		self.path_planner = path_planner.PathPlanner(obstacle_clearance=self.OBSTACLE_CLEARANCE)
	...
	def _inflate_hull(self, hull, margin):
		# Delegate to PathPlanner
		return self.path_planner.inflate_hull(hull, margin)

	def _collect_obstacle_hulls_for_path(self):
		# Delegate to PathPlanner; planner will use its configured obstacle_clearance
		return self.path_planner.collect_hulls(self.obstacles)

	def _segments_intersect(self, p1, p2, q1, q2):
		# Delegate to PathPlanner
		return self.path_planner._segments_intersect(p1, p2, q1, q2)

	def _dist_point_to_segment(self, p, a, b):
		return self.path_planner._dist_point_to_segment(p, a, b)
	
	def _segment_blocked_by_obstacles(self, p1, p2, obstacle_edges, tol=1e-9):
		return self.path_planner._segment_blocked_by_obstacles(p1, p2, obstacle_edges, tol=tol)

	def _dijkstra_path(self, adj, start_idx, goal_idx):
		return self.path_planner._dijkstra_path(adj, start_idx, goal_idx)

	def _plan_path_between_points(self, start, goal, hulls):
		# Delegate to PathPlanner
		return self.path_planner.plan_between_points(start, goal, hulls)
