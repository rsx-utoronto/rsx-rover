...
# Set up QOS profile (for state messages). The jetson side has a script in autonomy/config that can be imported as a QoSProfile. 
...
class mapOverlay(QWidget):
    def __init__(self, node):
        ...
        self.gps1 = message_filters.Subscriber(self.node, GnssSignalStatus, "calian_gnss/gps_extended")
        self.gps2 = message_filters.Subscriber(self.node, GnssSignalStatus, "calian_gnss/base_gps_extended")
    ...
    def gps_callback(self, gps1, gps2):
        lat1 = gps1.latitude
        ...
        self.viewer.headingSignal.emit(-gps1.heading)
        self.viewer.set_robot_position(avg_lat, avg_long)
        if self.centreOnRover == True:
            self.viewer.center_on_gps((avg_lat, avg_long))
...
class ArucoBar(QWidget):
    ...
    def bool_callback(self, msg):
        # Emit signal to update the label in the main thread
        print(f"[ArucoWidget] received aruco_found = {msg.data}")
        self.update_label_signal.emit(msg.data)
    ...
    def update_label(self, found):
        ...
        expected_name = f"AR {self.index}"

        if rising_edge and self.received_string == expected_name:
            self.assigned = True

        if self.assigned:
            self.label.setText(expected_name)
            self.label.setStyleSheet("""
                background-color: #4CAF50; 
                color: white;   
                border: 2px solid black; 
...
class StateMachineStatus(QWidget):
    ...
    def callback(self, msg):
        # Emit signal to update the label in the main thread
        print(f"[ArucoBar {self.index}] received aruco_found = {msg.data}")
        self.update_label_signal.emit(msg.data)
...
class LngLatEntryFromFile(QWidget):
    def __init__(self, node, map_overlay):
        ...
        self.submit_button.clicked.connect(self.collect_data)
        self.submit_button.clicked.connect(self.plot_points)
        self.submit_button.clicked.connect(self.send_data_to_rover)
        layout.addWidget(self.submit_button)
        self.setLayout(layout)
    ...
    def send_data_to_rover(self):
        script_path = str(Path("~/rover_ws/src/rsx-rover/scripts/GUI/transfer_obstacles.sh").expanduser())
        subprocess.run(["bash", script_path], check=True)
        script_path = str(Path("~/rover_ws/src/rsx-rover/scripts/GUI/transfer_lat_long_goal.sh").expanduser())
        subprocess.run(["bash", script_path], check=True)
...
class LngLatDeliveryEntryFromFile(QWidget):
    ...
    def collect_data(self):
        ...
        for line in lines:
            ...
            if draw_radius:
                radius_points.append(MapPoint(lat, lng, 20, name))
...
class CameraFeed:
    def __init__(self, node, label1, label2, label3, label4, splitter):
        ...
        self.state_sub = node.create_subscription(String, "state", self.state_callback, state_qos)
    ...
    def register_subscriber2(self):
        if self.image_sub2 is None:
            ...
            self.image_sub2 = node.create_subscription(CompressedImage, "/camera1/color/image_raw/compressed", self.callback2, 10) #this was camera2
    ...
    def bbox_callback(self, msg):
        if len(msg.data) == 8:
            ...
            if self.bbox:  # Only update last_bbox if we currently have a bbox to display
                self.last_bbox = self.bbox
            self.bbox = None  
    ...
    def update_image(self, data, label):
        ...
        if self.at_goal and self.last_bbox and not self.bbox:
...
class RoverGUI(QMainWindow):
    ...
    def __init__(self,node):
        super().__init__()
        self.node=node
        self.stm32_led_bridge = Stm32LedBridge(node)
    ...
    def string_signal_receive(self, msg):
        ...
        if self.reached_state is not None:
            ...
            self.camera_feed.at_goal = True
            if self.camera_feed.bbox:
                self.camera_feed.last_bbox = self.camera_feed.bbox
    ...
    def setup_split_screen_tab(self):
        ...
        auto_abort_button_motor_switch = QPushButton("Auto Abort 2")
        ...
        auto_abort_button_motor_switch.setStyleSheet(button_style)
        auto_abort_button_motor_switch.clicked.connect(self.pub_auto_abort_with_motor_switch)
        ...
        status_layout.addWidget(auto_abort_button_motor_switch)
        ...
        detection_group.setMinimumHeight(120)
        detection_group.setMaximumHeight(180)
        ...
        self.statusTermGroupBox.setMinimumHeight(60)
        ...
        main_vertical_splitter.addWidget(self.statusTermGroupBox)

        # Top area should get most of the space, status box starts smaller.
        main_vertical_splitter.setStretchFactor(0, 5)
        main_vertical_splitter.setStretchFactor(1, 1)

        # Optional starting sizes: top area large, status messages smaller.
        main_vertical_splitter.setSizes([650, 160])

        split_screen_layout = QVBoxLayout()
        split_screen_layout.addWidget(main_vertical_splitter)
    ...
    def pub_next_state(self):
        ...
        self.camera_feed.at_goal = False
    ...
    def pub_auto_abort_with_motor_switch(self):
        self.stm32_led_bridge._send_cmd("5")  # Send motor stop command to STM32
        time.sleep(1.0)  # Wait for 1 second before sending the next command
        self.stm32_led_bridge._send_cmd("4")  # Send motor start command to STM32
        self.pub_auto_abort()
...
class Stm32LedBridge:
    ...
    def __init__(self, node, ws_uri=None):
        self.node = node
        self.ws_host, self.ws_port = self._resolve_ws_server(ws_uri)
        ...
        self._skip_next_heartbeat = threading.Event()
        self._heartbeat_cmds = ("5", "4")
        ...
        self.led_sub = self.node.create_subscription(String, "led_light", self.led_state_callback, 10)

        if self.websockets is None:
            self.node.get_logger().warn(
                "websockets package is not installed; STM32 LED bridge is disabled."
            )
        else:
            self._start_server()

        # self.heartbeat_timer = self.node.create_timer(150.0, self._send_heartbeat)

    def _resolve_ws_server(self, ws_uri):
        env_uri = os.environ.get("STM32_WS_URI")
        env_host = os.environ.get("STM32_WS_HOST")
        env_port = os.environ.get("STM32_WS_PORT")

        host = env_host
        port = env_port

        uri = ws_uri or env_uri
        if uri:
            parsed = urlparse(uri)
            if parsed.hostname:
    ...
    def _run_server_loop(self):
        ...
        try:
            ...
            self.node.get_logger().warn(f"Failed to start STM32 LED server: {exc}")
        finally:
    ...
    async def _start_server_async(self):
        self.server = await self.websockets.serve(self._handler, self.ws_host, self.ws_port)
        self.node.get_logger().info(
            f"STM32 LED websocket server running on ws://{self.ws_host}:{self.ws_port}"
        )

    async def _handler(self, websocket):
        self.clients.add(websocket)
        self.node.get_logger().info("STM32 LED client connected")

        try:
            async for message in websocket:
                self.node.get_logger().info(f"STM32 LED RX: {message}")
                self._handle_stm32_message(message)
        except Exception as exc:
            self.node.get_logger().warn(f"STM32 LED websocket error: {exc}")
        finally:
            self.clients.discard(websocket)
            self.node.get_logger().info("STM32 LED client disconnected")

    def _handle_stm32_message(self, message):
        normalized = str(message).strip().lower()
        if normalized == "motor on":
            self._skip_next_heartbeat.set()
            self.node.get_logger().info("STM32 ACK received: motor on; skipping next heartbeat.")

    def led_state_callback(self, msg):
        raw_state = msg.data.strip().lower()
        normalized = raw_state.replace("_", " ")

        if normalized == self.last_state:
            return

        self.last_state = normalized
        desired_color = self.STATE_TO_COLOR.get(normalized)
        self._apply_color(desired_color)

    def _apply_color(self, color):
        if self.websockets is None:
            return

        desired_cmd = self.COLOR_TO_CMD.get(color)

        if desired_cmd == self.active_cmd:
            return

        if desired_cmd is None:
            if self.active_cmd is not None:
                self._send_cmd(self.active_cmd)
                self.active_cmd = None
            return

        if self.active_cmd is not None and self.active_cmd != desired_cmd:
            self._send_cmd(self.active_cmd)

        self._send_cmd(desired_cmd)
        self.active_cmd = desired_cmd

    def _send_cmd(self, command):
        if self.loop is None or self.loop.is_closed():
            self.node.get_logger().warn("STM32 LED server is not running; command dropped.")
            return

        future = asyncio.run_coroutine_threadsafe(self._broadcast(command), self.loop)
    ...
    def _send_heartbeat(self):
        ...
        if self._skip_next_heartbeat.is_set():
            self._skip_next_heartbeat.clear()
            self.node.get_logger().info("Skipping STM32 heartbeat sequence after ACK.")
            return

        for command in self._heartbeat_cmds:
            self._send_cmd(command)
        self.node.get_logger().info("Sent STM32 heartbeat sequence '5' then '4'.")

    def _handle_send_result(self, future):
        try:
            exc = future.exception()
        except Exception as exc:
            self.node.get_logger().warn(f"STM32 LED command failed: {exc}")
            return

        if exc:
            self.node.get_logger().warn(f"STM32 LED command failed: {exc}")

    async def _broadcast(self, command):
        if not self.clients:
            return

        await asyncio.gather(
            *(client.send(str(command)) for client in self.clients),
