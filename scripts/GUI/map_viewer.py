...
class MapViewer(QWidget):
    ...
    "OBJ 3":	resolve_path("icons/map_icon_OBJ.png"),
	}
	...
	def __init__(self, *args, **kwargs) -> None:
		...
		if not hasattr(self, '_bridge'):
			...
			print("Base current directory (used for finding csv files) is: ", base)
			...
			self.csv_perm = _choose_csv(
                'permanent_obstacles.csv',
                f'permanent_obstacles.csv',
		...
		(function(){{
			...
			layer.on('click', function(e) {{
				if (window.pyBridge && window.pyBridge.selectObstacle) {{
					window.pyBridge.selectObstacle(oid, cat, function() {{}});
				}}
			}});
			...
			layer.on('contextmenu', function(e) {{
				if (window.pyBridge && window.pyBridge.removeObstacle) {{
					window.pyBridge.removeObstacle(oid, cat, function() {{}});
				}}
			}});
				...
				if (oe && oe.button === 1) {{
					...
					ensureBridge(function() {{
						if (window.pyBridge && window.pyBridge.removePoint) {{
							window.pyBridge.removePoint('{pid}', {lat}, {lng}, '{key}', function() {{}});
						}}
					}});
					m.removeLayer(mk);
				}}
		...
		m.on('contextmenu', function(e) {{
			...
			function send(label) {{
				...
				if (window.pyBridge && window.pyBridge.storePoint) {{
					window.pyBridge.storePoint(lat, lng, label, function() {{}});
				}} else {{
		...
		(function() {{
			...
			mk.on('mousedown', function(e) {{
				...
				if (oe && oe.button === 1) {{
					...
					ensureBridge(function() {{
						if (window.pyBridge && window.pyBridge.removePoint) {{
							window.pyBridge.removePoint('{pid}', {lat}, {lng}, '{label_key}', function() {{}});
						}}
					}});
					m.removeLayer(mk);
				}}
	...
	def set_robot_position(self, lat: float, long: float) -> None:
		...
		if dist((lat, long), self.last_drawn_robot_position) > self.LINE_DRAW_THRESHOLD:
			...
			self._get_page().runJavaScript(jscode)
