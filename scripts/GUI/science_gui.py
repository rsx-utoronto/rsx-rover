...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QComboBox, QGridLayout, \
    ...
    QToolButton, QMenu, QAction, QLineEdit, QPushButton, QTextEdit, \
...
CACHE_DIR = Path("~/rover_ws/src/rsx-rover/scripts/GUI/tile_cache").expanduser()
...
class LngLatDeliveryEntryFromFile(QWidget):
    def __init__(self, map_overlay):
        ...
        self.submit_button = QPushButton("Get Science Data From File")
        self.submit_button.clicked.connect(self.collect_data)
        layout.addWidget(self.submit_button)
        self.setLayout(layout)
    ...
    def collect_data(self):
        # Read data from the file
        # file_path = Path(__file__).parent.parent.parent.resolve() / "science_lat_lon_goal.csv"
        file_path = Path("~/rover_ws/src/rsx-rover/science_lat_lon_goal.csv").expanduser()
        with open(file_path, 'r') as file:
            lines = file.readlines()
...
class CamerasWindow(QDialog):
    ...
    def __init__(self, node, parent=None):
        ...
        # -----------------------------
        self.camera1_topic = "/zed/zed_node/rgb/image_rect_color/compressed"   # TODO: replace with real topic
        self.camera2_topic = "/camera1/color/image_raw/compressed"   # TODO: replace with real topic
        self.camera3_topic = "/webcam3/compressed"   # TODO: replace with real topic
        ...
        # If parent GUI provided, reuse its shared map overlay; otherwise create one
        if parent is not None and hasattr(parent, 'map_overlay'):
            self.map_overlay = parent.map_overlay
        else:
            self.map_overlay = mapOverlay(self.node)

        # Map group for cameras window
        map_group = QGroupBox("Map")
        map_layout = QVBoxLayout()
        map_ctrl_layout = QHBoxLayout()

        self.map_checkbox = QCheckBox("Recentre map when rover offscreen")
        self.map_checkbox.setChecked(False)
        self.map_checkbox.stateChanged.connect(lambda state: setattr(self.map_overlay, 'centreOnRover', state == Qt.Checked))

        self.clear_map_button = QPushButton("Clear Map")
        self.clear_map_button.clicked.connect(self.map_overlay.clear_map)

        map_ctrl_layout.addWidget(self.map_checkbox)
        map_ctrl_layout.addStretch(1)
        map_ctrl_layout.addWidget(self.clear_map_button)
...
class RoverGUI(QMainWindow):
    ...
    def __init__(self, node:Node):
        self.node=node
        super().__init__()
        self.node.get_logger().info("Initializing RoverGUI")
        self.statusTerminal = statusTerminal()
        self.setWindowTitle("Rover Control Panel")
        self.setGeometry(100, 100, 1200, 800)
        # Fullscreen state
        self._fullscreen = False
        print("[science_gui] Initializing RoverGUI with node:", node)

        # Menu: View -> Toggle Fullscreen (F11)
        ...
        print(f"[science_gui] created publisher for /science_state on node '{getattr(node, 'name', getattr(node, 'get_name', lambda: 'unknown')())}' -> {self.science_state_pub}")
        ...
        # Create a shared map overlay so it can be reused outside the Science tab
        # (LngLatEntryBar, CamerasWindow, etc.)
        self.map_overlay = mapOverlay(self.node)

        self.cameras_window = CamerasWindow(self.node, self)

        # Setup tabs before adding them
        # Keep science tab UI initialisation (not added to tabs) so its components
        # remain available to the rest of the program if needed.
        self.setup_science_tab()  # Setup science tab first (UI created but tab not added)
        ...
        # Add tabs to QTabWidget (removed Science and State Machine tabs)
        # Note: scienceTab and longlat_tab UI are still created above but not shown
        ...
        self.tabs.addTab(self.longlat_tab, "Lng/Lat Entry")
    ...
    def science_state_callback(self, msg):
        ...
        # self.node.get_logger().info("in science state callback")
        ...
        if msg.receiver == "rsx":
            ...
            # self.node.get_logger().info(f"Science state update - Peripheral: {self.state_peripheral}, Command: {self.state_command}, Response: {self.state_response}, Data: {self.state_data}")
            ...
            if self.state_peripheral == "ultrasonic": 
                display_text = f"Ultrasonic: {self.state_data[0]:.2f} cm"
                self.cameras_window.ultrasonicUpdateSignal.emit(display_text)
            if self.state_peripheral == "nano-environmental-sensor1":
                # print("received nano sens 2")
                ...
                try:
                    ...
                    return
                # self.node.get_logger().info(f"Received Nano-Environmental-Sensor2 data: {temp_value}, {hum_value}")

                self.temp_data_buffer.append(temp_value)
                self.temp_time_buffer.append(self.temp_time)
                self.temp_time += 1

                self.hum_data_buffer.append(hum_value)
                ...
                self.temp_save_data = self.temp_plot_data
                self.hum_save_data = self.hum_plot_data
                self.probeUpdateSignal.emit(True)
                # Mirror nano sensor samples into the Chem+Temp/Humidity Site 1 plots.
                self.chem_graph_site_1_callback()
                # self.node.get_logger().info(f"Updated temp/hum data buffers and emitted probe update signal{self.temp_plot_data}")
            elif self.state_peripheral == "nano-environmental-sensor2":
                try:
                    temp_value = float(self.state_data[0])
                    hum_value = float(self.state_data[1])
                ...
                self.hum_save_data = self.hum_plot_data
                self.probeUpdateSignal.emit(True)
                # Mirror nano sensor samples into the Chem+Temp/Humidity Site 2 plots.
                self.chem_graph_site_2_callback()
    ...
    def multispectral_move_servo_forward(self):
        ...
        print(f"Publishing servo forward command: {send}")
    ...
    def setup_cams_tab(self):
        ...
        self.camerasplitter_cams_tab.setStretchFactor(0, 1)
        self.camerasplitter_cams_tab.setStretchFactor(1, 1)
        self.camerasplitter_cams_tab.setStretchFactor(2, 1)
        self.camerasplitter_cams_tab.setStretchFactor(3, 1)
        self.camerasplitter_cams_tab.setStretchFactor(4, 1)
        ...
        cam_tab_layout.setContentsMargins(0, 0, 0, 0)
        cam_tab_layout.setSpacing(0)
        # split_screen_layout.addWidget(splitter)
        cam_tab_layout.addWidget(camera_group, 1)
        # split_screen_layout.addWidget(self.statusTermGroupBox) 
        self.camsTab.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def setup_science_tab(self):
        ...
        # Reuse shared map_overlay if already created on the main GUI instance
        if not hasattr(self, 'map_overlay'):
            self.map_overlay = mapOverlay(self.node)
        ...
        # Map was moved to the Cameras window. Keep a placeholder here
        placeholder_label = QLabel("Map moved to Cameras window")
        placeholder_label.setAlignment(Qt.AlignCenter)
        placeholder_label.setStyleSheet("color: gray; font-style: italic;")
        right_top_layout.addWidget(placeholder_label)
...
if __name__ == '__main__':
    ...
    print("ROS2 spinning in Qt event loop")
    try:
        sys.exit(app.exec_())
    finally:
        node.destroy_node()
