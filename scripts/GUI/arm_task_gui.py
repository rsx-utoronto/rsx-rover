...
class CameraFeed:
    ...
    def register_subscriber1(self):
        if self.image_sub1 is None:
            ...
            self.image_sub1 = self.node.create_subscription(CompressedImage, "/zed_node/rgb/image_rect_color/compressed", self.callback1, 10)
    ...
    def register_subscriber2(self):
        if self.image_sub2 is None:
            ...
            self.image_sub2 = self.node.create_subscription(CompressedImage, "/camera2/camera/color/image_raw/compressed", self.callback2, 10)
...
class RoverGUI(QMainWindow):
    ...
    def setup_lngLat_tab(self):
        self.lngLatEntry = LngLatEntryBar(self.map_overlay_splitter, self)
        self.lngLatFile = LngLatEntryFromFile(self.map_overlay_splitter, self)
...
if __name__ == '__main__':
    ...
    rclpy.create_node('rover_gui', anonymous=False)
