...
# Camera setup with stable dimensions
cap = cv2.VideoCapture(1)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 384)

def process_depth(frame):
    """Process depth map with dimension validation"""
...
def enhance_image(frame, depth_map):
    """Fixed depth-aware enhancement"""
    ...
    depth_weight = np.clip(depth_map * 2, 0, 1)[..., np.newaxis]
    blended = enhanced * 0.7 + sharpened * 0.3 * depth_weight
...
while True:
    ...
    processed_frame = enhance_image(frame, depth_map)
    ...
    cv2.imshow("Fixed Depth-Aware Detection", frame)
