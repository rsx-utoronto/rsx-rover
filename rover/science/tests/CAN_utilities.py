...
def float_to_sparkdata(f : float) -> list:
    """
    float -> list(int)

    Takes in a float value (number of rotations if position value or rpm if velocity value) 
    and returns the data packet in the form that SparkMAX requires
    ...
    # FYI: Conversion of the packed struct to unsigned int is done because 
    #      sometimes the packed struct converts the byte to a valid ASCII character (if the 
    #      ASCII character exists)
    # e.g. >>> f = 103.5
    #      >>> struct.pack('<f', f)
    #      b'\x00\x00\xcfB'
    #      >>> hex(struct.unpack('<I', struct.pack('<f', f))[0])
    #      '0x42cf0000'
    # Note that the hex value from hex() function has 0x42 as the most signficant byte
    # but the struct.pack gives the letter "B" because 0x42 as a byte in ASCII is "B"

    # struct instructions:
    #   struct.pack(format, v1, v2...)
    #       - v1, v2... are values that are packed according to the format specified 
    #         by the first argument
    #       - For full details on format, see the documentation
    #         "<" implies little endian, ">" implies big endian
    #         "f" implies the values are in float, "I" implies values are in unsigned int
    #   struct.unpack(format, buffer)
    #       - format has the same rules as struct.pack() case
    #       - buffer is a bytes array (the data type is the same as the 
    #         output of struct.pack())
    #       - Note that output of this function is always a tuple, even 
    #         if it is only a single element, that is why [0] is there at the end
    
    input_hex =  hex(struct.unpack('<I', struct.pack('<f', f))[0])

    # float values are 32 bits (8 hex values), so there should be 
    # 10 elements in the input_hex string. If there are not, we know 
    # the first two elements of input_hex are "0x" and the rest are the 
    # significant bits. The remaining bits are assumed to be zero so we pad the result with 0
    if len(input_hex) != 10:
        input_hex = input_hex[:2] + input_hex[2:] + (10-len(input_hex))*'0'
    
    # Preparing the data to send in little endian format
    return [eval('0x'+input_hex[-2:]), eval('0x'+input_hex[-4:-2]),
            eval('0x'+input_hex[-6:-4]), eval('0x'+input_hex[-8:-6]),
            0x00, 0x00, 0x00, 0x00]

# def vel_to_sparkdata(vel : int) -> list:
#      """
#      Docstring for vel_to_sparkdata
