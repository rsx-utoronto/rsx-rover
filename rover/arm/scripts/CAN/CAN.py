...
class CAN_Node():
	...
	# CAN Node only
	def generate_can_id(self, dev_id : int, api : int, 
	...
	# CAN Node only
	def pos_to_sparkdata(self, f : float):
		"""
		float -> list()
	...
	# CAN Node only
	def sparkfixed_to_float(self, fixed : int, frac : int = 5):
		"""
	...
	# CAN Node only
	def initialize_bus(self, channel= 'can0', interface= 'socketcan'):
		"""
		(str, str) -> (void)
	...
	# CAN Node only
	def send_can_message(self, can_id : int, data = None, ext = True, err = False, rtr = False):
		"""
	...
	# CAN Node only
	def read_can_message(self, data, api):
		"""
		...
		*******NEEDS TO BE COMPLETED*******
		...
		if api:
			...
			# motor current every 20ms. We only need current
				...
				currVal_float = self.sparkfixed_to_float(currVal_fixed)
	...
	def calc_differential(self, d_pos, d_angle):
		"""
	...
	# CAN Node only
	def generate_data_packet(self, data_list : list):
		"""
		...
		data_list[-3], data_list[-2], correction = self.calc_differential(data_list[len(data_list)-2], data_list[len(data_list)-1])
		...
		for angle in data_list:
			...
			spark_data.append(self.pos_to_sparkdata(angle))
		...
		while not rospy.is_shutdown():
			...
			for i in range(1, len(spark_input)+1):
				...
				if motor_num > 10 and motor_num < 18:
					id = self.generate_can_id(dev_id= motor_num, api= CMD_API_POS_SET)
					self.send_can_message(can_id= id, data= spark_input[i - 1])
				
				else:
					break

			# Parse updated motor angles and current
			self.read_message_from_spark()
			...
			self.LMS_pub.publish(limit_switch_msg.data)
