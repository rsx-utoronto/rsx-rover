    ...
    def motor_controller(self, angle_from_rover):
        if angle_from_rover >= 0:
            ...
            if angle_from_rover < 85:
                self.angular_velocity = self.ref_ang_vel * abs(math.cos(math.radians(abs(angle_from_rover))))
                self.lin_vel = 0.3
            #For large values just turn
            else:
                self.angular_velocity = self.ref_ang_vel
        ...
        # print("Left speed: " + str(self.left_speed) + " Right speed: " + str(self.right_speed))