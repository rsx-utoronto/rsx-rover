...
class Aimer: # 
    ...
    def update(self, aruco_top_left: tuple, aruco_top_right: tuple,  #determines what the speeds should be
        ...
        if (aruco_top_left is None or aruco_top_right is None or aruco_bottom_left is None or aruco_bottom_right is None):
            print("no aruco detection\n\n")
            return
        
        aruco_x = (aruco_top_left[0] + aruco_top_right[0] + aruco_bottom_left[0] + aruco_bottom_right[0]) / 4
        print ("Aruco coordinates (top left, top right, bot left, bot right): ", aruco_top_left, aruco_top_right, aruco_bottom_left, aruco_bottom_right)
        print("the center x-coordinate of the aruco marker is ", aruco_x)
        ...
        if aruco_distance_est < self.min_aruco_area:
            ...
            print ("moving forward. Aruco is too far\n\n")
            out_linear = 1
        else:
            print ("not moving forward. Aruco is close enough\n\n")
            out_linear = 0
...
class AimerROS(Aimer):  #updates coords continuously 
    ...
    def snapshot_bbox(self) -> tuple[tuple[float, float], tuple[float, float], tuple[float, float], tuple[float, float]]:
        with self._lock:
