...
from rover_qos import STATE_QOS # For QoS adjustments.
...
class GLOB_MSGS(Node):
    def __init__(self):
        ...
        self.pub = self.create_publisher(String, "gui_status", STATE_QOS)
        self.state_publisher = self.create_publisher(String, "state", STATE_QOS)
        self.led_publisher = self.create_publisher(String, "led_light", 10)
        self.mission_state_pub = self.create_publisher(MissionState, 'mission_state', 10) #remember that this is different
        self.set_origin_pub = self.create_publisher(Pose, '/set_origin', 10) #EDWARD FIX PLEASE
        self.abort_pub = self.create_publisher(Bool, "/auto_abort_check", 10)
    ...
    def feedback_callback(self, msg):
        ...
        if msg.state == "ARUCO_REACHED":
            self.gs_status = "ARUCO_REACHED"
    ...
    def execute(self, userdata):
        ...
        if path != {}: #Checks if all locations are visited
            ...
            try:
                ...
                # while self.glob_msg.sla_status not in {"SLA_DONE", "SLA_AVOIDANCE_DONE", "SLA_FAILED"} and rclpy.ok():
...
class GNSSState(smach.State):
    ...
    def execute(self, userdata):
        ...
        while not self.glob_msg.get_next_task_check() and not self.glob_msg.abort_check:
    ...
    def execute(self, userdata):
        ...
        if current_distance < 5:
            ...
            if not self.glob_msg.done_early:
                ...
                while self.glob_msg.gs_status is None and rclpy.ok() and not self.glob_msg.abort_check:
                    time.sleep(1)
                ...
                ar_in_correct_loc = (self.glob_msg.gs_status == "ARUCO_REACHED")
                self.glob_msg.pub_state(String(data=f"End of {self.state_name} grid search {self.glob_msg.gs_status}"))
                ...
                if ar_in_correct_loc:
                    self.glob_msg.pub_state(String(data=f"Grid Search found {self.state_name}"))
    ...
    def execute(self, userdata):
        ...
        if current_distance < 5:
            ...
            if not self.glob_msg.done_early:
                ...
                while self.glob_msg.gs_status not in {"OBJ_FOUND", "OBJ_NOT_FOUND"} and rclpy.ok() and not self.glob_msg.abort_check:
                    time.sleep(1)
                    self.glob_msg.pub_state(String(data=f"Waiting for grid search status for object, global gs_status {self.glob_msg.gs_status}"))
                ...
                if self.mallet_found or self.waterbottle_found or self.hammer_found:
                    if self.mallet_found:
                        self.glob_msg.pub_state(String(data="Grid Search found Mallet"))
                    if self.waterbottle_found:
                        self.glob_msg.pub_state(String(data="Grid Search found Waterbottle"))
                    if self.hammer_found:
                        self.glob_msg.get_logger().info("Grid Search found Hammer")
                        self.glob_msg.pub_state(String(data="Grid Search found Hammer"))
...
class ABORT(smach.State):  # Assuming it won't be called before we try to go to a task 
    ...
    def execute(self, userdata):
        # NOTE: Keep abort_check TRUE during the return journey so other nodes see the abort signal
        # and can respond appropriately (e.g., stop grid search, halt obstacle avoidance).
        # Only clear it after the return SLA completes.
        
        # Determine the location to return to.
        # Use userdata.prev_loc which tracks the actual previous location we came from,
        # NOT the position in RUN_STATES (which assumes missions run in order).
        # This handles cases where the operator skips tasks or runs them out of order.
        return_state = userdata.prev_loc
        
        if return_state is None or return_state == "start":
            self.glob_msg.pub_state(String(data="No previous location available; returning to start location."))
            return_location = userdata.start_location if userdata.start_location is not None else (0, 0)
            self.glob_msg.pub_state(String(data=f"Returning to start location {return_location}"))
        else:
            self.glob_msg.pub_state(String(data=f"Aborting back to previous state {return_state}"))
            return_location = userdata.locations_dict.get(return_state)
            if return_location is None:
                self.glob_msg.pub_state(String(data=f"Missing return target for {return_state}; falling back to start location."))
                return_location = userdata.start_location if userdata.start_location is not None else (0, 0)

        self.glob_msg.pub_state(String(data=f"Aborting to location: {return_location} to {return_state}"))
        
        # NOTE: Do NOT remove the aborted task from rem_loc_dict. Keep it so the operator can resume it later
        # if needed (e.g., if abort was accidental or the situation has changed).
        # Only pop tasks when they are actually completed normally in other states.
        ...
        try:
            ...
            self.glob_msg.abort_check = False # Clear abort check to allow SLA to proceed, but keep abort_pub True until SLA completes
            self.glob_msg.abort_pub.publish(Bool(data=False))
            self.sla_status = None  # Clear SLA status before starting
            use_avoidance = return_state in {"OBJ2", "OBJ3"} or userdata.aborted_state in {"OBJ2", "OBJ3"}
            abort_msg = MissionState()
            abort_msg.state = "START_SL_AVOIDANCE" if use_avoidance else "START_SL"
            abort_msg.current_state = return_state
            abort_msg.current_goal = PoseStamped()
            abort_msg.current_goal.pose.position.x = float(return_location[0])
            abort_msg.current_goal.pose.position.y = float(return_location[1])
            self.glob_msg.sla_status = None
            self.glob_msg.mission_state_pub.publish(abort_msg)

            # Wait for SLA completion. Include all possible completion states:
            # - "SLA_DONE": straight-line completed successfully
            # - "SLA_FAILED": straight-line failed to reach goal
            # - "SLA_AVOIDANCE_DONE": obstacle-avoiding straight-line completed (if using A*)
            # - "SLA_AVOIDANCE_FAILED": obstacle avoidance failed
            while self.glob_msg.sla_status not in {"SLA_DONE", "SLA_FAILED", "SLA_AVOIDANCE_DONE", "SLA_AVOIDANCE_FAILED"} and rclpy.ok():
                time.sleep(1)
            ...
            # IMPORTANT: After abort completes, we need to STOP and wait for user confirmation
            # via the "Next Task" button before proceeding. This ensures the rover doesn't
            # immediately resume the next task automatically.
            self.glob_msg.pub_state(String(data="Abort complete. Press 'Next Task' button to resume mission."))
            self.glob_msg.pub_led_light(String(data="auto"))
            
            # Reset next_task_check flag to ensure we wait for a fresh button press
            self.glob_msg.next_task_check = False
            
            # Wait for operator to press Next Task button
            while not self.glob_msg.get_next_task_check() and rclpy.ok():
                self.glob_msg.pub_state(String(data="Waiting for Next Task Button to resume mission"))
                time.sleep(1)
            
            # Clear the next_task_check flag for the next task
            self.glob_msg.next_task_check = False
            ...
            # Ensure abort flag is cleared even if exception occurs
