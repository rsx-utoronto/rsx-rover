...
class StraightLineApproach(Node):
    ...
    def move_to_target(self, target_x, target_y, state="Location Selection"): #navigate needs to take in a state value as well (FINISH IT)
        ...
        print(f"Current position: ({self.x:.3f}, {self.y:.3f}), Heading: {self.heading:.3f}")
        while (rclpy.ok()) and (self.abort_check is False):
            # print(f"in move to target, self.x: {self.x:.3f}, self.y: {self.y:.3f}, self.heading: {self.heading:.3f}, target_x: {target_x:.3f}, target_y: {target_y:.3f}")
            msg = Twist()
