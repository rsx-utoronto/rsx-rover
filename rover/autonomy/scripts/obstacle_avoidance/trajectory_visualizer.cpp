...
void Visualizer::publishTrajectories(const std::vector<std::vector<Pose2D>>& trajectories)
{
...
void Visualizer::publishTrajectory(const std::vector<Pose2D>& trajectory)
{
    visualization_msgs::Marker line_marker;
    line_marker.header.frame_id = frame_id_;
    line_marker.header.stamp = ros::Time::now();
